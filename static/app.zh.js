...
const MAX_TASKS = 50;
const tasks = [];
// Purge legacy demo tasks from localStorage
for (let i = tasks.length - 1; i >= 0; i--) { if (tasks[i].demo) tasks.splice(i, 1); }

// Demo tasks (assets hosted on TOS — tutorial/ prefix, excluded from cleanup)
const DEMO_TASKS = {
  all: {
    id: "demo-reference-zh", demo: true, status: "completed",
    created: 0, endTime: 0,
    videoUrl: "https://anyfast-seedance-web.tos-cn-beijing.volces.com/tutorial/1775538500106_f97f0fab.mp4",
    input: {
      prompt: "@图片1 @图片2 模仿 @视频1 的动作和步伐，音乐参考 @音频1。",
      model: "seedance", refMode: "all", ratio: "16:9", duration: 10,
      mediaItems: [
        { type: "image", name: "@图片1.png", url: "https://anyfast-seedance-web.tos-cn-beijing.volces.com/tutorial/1775538496964_246bb024.png", cosUrl: "https://anyfast-seedance-web.tos-cn-beijing.volces.com/tutorial/1775538496964_246bb024.png" },
        { type: "image", name: "@图片2.png", url: "https://anyfast-seedance-web.tos-cn-beijing.volces.com/tutorial/1775538497854_43f394ba.png", cosUrl: "https://anyfast-seedance-web.tos-cn-beijing.volces.com/tutorial/1775538497854_43f394ba.png" },
        { type: "video", name: "@视频1.mp4", url: "https://anyfast-seedance-web.tos-cn-beijing.volces.com/tutorial/1775538498673_cd8c740b.mp4", cosUrl: "https://anyfast-seedance-web.tos-cn-beijing.volces.com/tutorial/1775538498673_cd8c740b.mp4" },
        { type: "audio", name: "@音频1.wav", url: "https://anyfast-seedance-web.tos-cn-beijing.volces.com/tutorial/1775538499332_26f377aa.wav", cosUrl: "https://anyfast-seedance-web.tos-cn-beijing.volces.com/tutorial/1775538499332_26f377aa.wav" },
      ]
    },
    response: {}
  },
  keyframes: {
    id: "demo-keyframes-zh", demo: true, status: "completed",
    created: 0, endTime: 0,
    videoUrl: "https://anyfast-seedance-web.tos-cn-beijing.volces.com/tutorial/1775538514705_b64603b2.mp4",
    input: {
      prompt: "银色魔法爆发，铠甲在闪光中迅速贴合她的身体。镜头先推进再拉远。头发飞扬，冲击波掠过海面。月光海滩，扁平矢量风格。",
      model: "seedance", refMode: "keyframes", ratio: "16:9", duration: 5,
      kfFirst: { url: "https://anyfast-seedance-web.tos-cn-beijing.volces.com/tutorial/1775538500977_54f9e284.png", cosUrl: "https://anyfast-seedance-web.tos-cn-beijing.volces.com/tutorial/1775538500977_54f9e284.png", name: "frame_start.png" },
      kfLast: { url: "https://anyfast-seedance-web.tos-cn-beijing.volces.com/tutorial/1775538513929_fe818ff6.png", cosUrl: "https://anyfast-seedance-web.tos-cn-beijing.volces.com/tutorial/1775538513929_fe818ff6.png", name: "frame_end.png" },
    },
    response: {}
  }
};
...
let kfFirst = null;
let kfLast = null;
// 增量渲染缓存（init IIFE 在函数定义之前就会调 renderTasks，所以放到顶层）
...
function updateCostLabel() {
  ...
  const cost = (base * dur * REGION_MULT).toFixed(2);
  ...
  if (p.compose) {
    ...
    try {
      ...
      if (c.refMode) {
        ...
        const labels = { all: "参考", keyframes: "首尾帧" };
        const refLabel = document.getElementById("ref-label");
        if (refLabel) refLabel.textContent = labels[c.refMode] || "参考";
...
(function init() {
  ...
  ta.addEventListener("paste", (e) => {
    ...
    const html = esc(text).replace(/@(图片|视频|音频)(\d+)/g, (match, type, num) => {
      const mType = type === "图片" ? "image" : type === "视频" ? "video" : "audio";
  ...
  if (!document.getElementById("i-base").value) {
    document.getElementById("i-base").value = "https://www.anyfast.com.cn";
  }
...
function closeAllPopups() {
  ...
  // Keep at-pill active if asset panel is open
...
function syncRefMode() {
  ...
  document.getElementById("i-prompt").setAttribute("data-placeholder", isKf
    ? "描述两帧之间的动作或平滑过渡。"
    : "上传 1-12 个素材并输入提示词。可混合使用图片、视频和音频。示例：@图片1 模仿 @视频1 的动作，配音参考 @音频1。");
...
function buildMentionList() {
  ...
  el.innerHTML = mediaItems.map((m, i) => {
    const typeLabel = m.type === "image" ? "图片" : m.type === "video" ? "视频" : "音频";
...
function setPromptText(text) {
  ...
  el.innerHTML = esc(text).replace(/@(图片|视频|音频)(\d+)/g, (match, type, num) => {
    const mType = type === "图片" ? "image" : type === "视频" ? "video" : "audio";
    const idx = parseInt(num);
    let count = 0;
    const item = mediaItems.find(m => m.type === mType && ++count === idx);
...
function pickFile() {
  ...
  input.onchange = () => {
    ...
    renderStack();
    if (skipped) alert("已达上限：最多 9 张图片、3 个视频、3 个音频");
  };
  input.click();
}
...
function removeMediaItem(id) {
  ...
  // Compute the removed item's type-index (1-based) within its type
  let typeIdx = 0;
  for (let i = 0; i <= idx; i++) {
...
function updateMentionsAfterRemove(removedType, removedIdx) {
  const typeLabel = { image: "图片", video: "视频", audio: "音频" }[removedType];
  if (!typeLabel) return;
  ...
  for (const span of mentions) {
    ...
    if (n === removedIdx) {
      // Remove the mention (and a trailing nbsp/space if present)
      ...
      // Update the trailing text node inside the span (last child is the text "@图片N")
...
function startPolling() {
  if (pollTimer) return;
  if (document.hidden) return; // visibilitychange 会在标签可见时再启动
  pollAll();
...
function renderTaskInput(t) {
  ...
  h += `<div class="task-input-header"><span style="font-size:10px;color:#c0c4ca;letter-spacing:.3px">输入</span><span class="task-input-reuse"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="1 4 1 10 7 10"/><path d="M3.51 15a9 9 0 105.64-11.36L1 10"/></svg> 复用</span></div>`;
  if (inp.prompt) {
    const media = inp.mediaItems || [];
    const promptHtml = esc(inp.prompt).replace(/@(图片|视频|音频)(\d+)/g, (match, type, num) => {
      const mType = type === "图片" ? "image" : type === "视频" ? "video" : "audio";
      const idx = parseInt(num);
      let count = 0;
  ...
  if (inp.refMode === 'keyframes') {
    if (inp.kfFirst) thumbs.push({ type:'image', url:inp.kfFirst.cosUrl||inp.kfFirst.url, label:'首帧' });
    if (inp.kfLast) thumbs.push({ type:'image', url:inp.kfLast.cosUrl||inp.kfLast.url, label:'尾帧' });
  ...
  const rl = inp.refMode === 'keyframes' ? '首尾帧' : '参考';
...
function restoreInput(taskId) {
  ...
  const refLabels = { all:"参考", keyframes:"首尾帧" };
  document.getElementById("ref-label").textContent = refLabels[inp.refMode] || "参考";
  document.querySelectorAll("#ref-popup .popup-item").forEach(item => {
...
function renderOneTask(t) {
  ...
  const demoBadge = t.demo ? `<span style="font-size:10px;font-weight:500;color:#3d9be9;background:#eef6ff;padding:2px 7px;border-radius:10px">教程</span>` : "";
  const removeBtn = t.demo
    ? `<button class="task-remove" onclick="dismissDemo('${escJs(t.id)}')" title="删除">×</button>`
    : `<button class="task-remove" onclick="removeTask('${escJs(key)}')" title="删除">×</button>`;
  ...
  if (!t.demo) h += `<details class="task-json"${detailsOpen}><summary>响应数据</summary><div class="rj">${esc(JSON.stringify(t.response,null,2))}</div></details>`;
  h += `</div></div>`;
  return h;
}

// 增量渲染：只重建签名变化的卡片，避免 6 秒轮询期间整个列表抖动 / 视频重新加载
// _renderedTasks 声明在文件顶部（init IIFE 会在此处之前调用 renderTasks）
...
function renderTasks() {
  ...
  if (!list.length) {
    ...
    c.innerHTML = '<p class="empty">暂无任务</p>';
    return;
  }
...
function clearTasks(){if(!confirm("确认清空所有任务记录？"))return;tasks.length=0;saveTasks();renderTasks()}
// Strip noise (request id, trailing parenthetical identifiers) so translation matching is cleaner.
function stripErrorNoise(msg) {
  if (!msg) return "";
  return String(msg)
...
// Translate a raw error string to Chinese. Returns null if no rule matched —
// callers can then decide how to fall back.
function translateError(raw) {
  ...
  if (msg.includes("无效的令牌") || msg.includes("无效令牌") || m.includes("invalid token"))
    return "API Key 无效，请检查设置或更换有效的 Key";
  if (msg === "API Key is required" || msg === "API Key required")
    return "请先填写 API Key";
  if (m.includes("401") || m.includes("unauthorized"))
    return "API Key 无效，请检查设置";
  if (m.includes("403") || m.includes("forbidden"))
    return "权限不足，无法访问该接口";
  if (m.includes("invalid api base") || m.includes("only https base urls"))
    return "API Base URL 无效，请填写 https 开头的公网地址";

  // ── AnyFast proxy specific ─────────────────────────────────
  if (m.includes("unsupported asset action"))
    return "该素材操作暂不支持，请联系服务管理员开通";
  if (msg.includes("价格未配置") || m.includes("price not set") || m.includes("price not configured"))
    return "模型计费未配置，请联系服务管理员";
  if (m.includes("get_channel_failed") || msg.includes("可用渠道不存在"))
    return "服务渠道暂时不可用，请稍后重试";
  if (m.includes("quota") || m.includes("insufficient balance") || m.includes("out of credit")
      || msg.includes("额度") || msg.includes("余额不足"))
  ...
  if (m.includes("resolve asset") || msg.includes("素材处理中"))
    return "素材处理中，请稍后重试";
  if (msg.includes("Asset whitelisting failed") || msg.includes("asset whitelisting failed"))
    return "素材加白失败，请更换图片后重试";
  if (msg.includes("Asset whitelisting timeout"))
    return "素材加白超时，请稍后重试";
  if (msg.includes("Asset not found") || msg.includes("asset not found"))
    return "未找到该素材";
  if (msg.includes("storageUrls array required"))
    return "素材参数错误";

  // ── Rate / quota / timeout ─────────────────────────────────
  if (m.includes("rate limit") || m.includes("ratelimit") || m.includes("too many requests") || m.includes("429"))
    return "请求过于频繁，请稍后重试";
  if (m.includes("timeout") || m.includes("timed out"))
    return "服务繁忙，请稍后重试";

  // ── Network ────────────────────────────────────────────────
  if (m.includes("networkerror") || m.includes("failed to fetch") || m.includes("network error") || m === "网络错误")
    return "网络连接失败，请检查网络";
  ...
  // ── Upload ─────────────────────────────────────────────────
  if (m.includes("upload failed") || msg === "上传失败")
    return "上传失败，请重试";
  if (msg === "获取上传地址失败")
    return msg;
...
function friendlyError(msg) {
  ...
  if (cleaned.length > 80) return "生成失败：" + cleaned.slice(0, 60) + "…";
  return "生成失败：" + cleaned;
}
...
// Lookup asset on server by content hash; returns asset record or null
...
async function uploadAssetWithHash(file, onProgress) {
  ...
  if (!presignResp.ok) throw new Error("获取上传地址失败");
  const presign = await presignResp.json();
  // 3) XHR PUT with progress
...
// Returns "asset://<id>" if the library has a ready entry, empty string otherwise.
...
function renderAssetGrid() {
  ...
  if (!el) return;
  if (assetLibrary.length === 0) {
    el.innerHTML = '<div class="asset-empty">点击「上传」添加素材到素材库<br>素材库独立于输入框，不会被清除</div>';
    return;
  }
  const statusLabel = { ready: "已加白", pending: "加白中…", failed: "失败", none: "未加白", uploading: "上传中…" };
  el.innerHTML = assetLibrary.map(a => {
    ...
    const wlOverlay = canWl
      ? `<div class="asset-card-wl show"><button onclick="event.stopPropagation();whitelistAsset(${a.id})">加白</button></div>` : '';
    ...
    return `<div class="asset-card${isUploading ? ' uploading' : ''}" ${clickAttr} ${dragAttrs} title="${esc(a.name || '素材')}">
      ${thumb}
      <span class="asset-card-status ${a.asset_status}">${statusLabel[a.asset_status]}</span>
      ${removeBtn}
      <div class="asset-card-name">${esc(a.name || "素材")}</div>
      ${wlOverlay}
      ${progressBar}
    </div>`;
  }).join("");
...
async function handleAssetUpload(files) {
  console.log("[AssetUpload] handleAssetUpload called, files:", files?.length);
  if (!files || !files.length) { console.warn("[AssetUpload] no files"); return; }
  const key = document.getElementById("i-key").value.trim();
  if (!key) {
    // Cache files and re-invoke after key is set
    const cached = Array.from(files);
    showKeyModal(() => handleAssetUpload(cached));
    return;
  }
  // Copy file list before any clearing (FileList is live)
  const fileList = Array.from(files);
  // Reset input so selecting the same file again triggers change
  setTimeout(() => { const el = document.getElementById("asset-file-input"); if (el) el.value = ""; }, 0);
  for (const file of fileList) {
    console.log("[AssetUpload] Processing:", file.name, file.type, file.size, "bytes");
    ...
    try {
      ...
      if (reused && asset) {
        // Already in library — promote to top and drop placeholder
        const idx = assetLibrary.findIndex(a => a.id === placeholderId);
        if (idx >= 0) assetLibrary.splice(idx, 1);
        // Remove any existing duplicate, then unshift
        const existingIdx = assetLibrary.findIndex(a => a.id === asset.id);
        if (existingIdx >= 0) assetLibrary.splice(existingIdx, 1);
        assetLibrary.unshift(asset);
        console.log("[AssetUpload] Reused existing asset:", asset.id);
      } else {
        // Register in DB with contentHash
        ...
        if (regResp.ok) {
          ...
          throw new Error("注册素材失败: " + errText);
        }
        if (idx >= 0 && assetLibrary[idx].id === placeholderId) assetLibrary.splice(idx, 1);
      }
    } catch (e) {
      console.error("[AssetUpload] Upload failed:", file.name, e.message);
      const idx = assetLibrary.findIndex(a => a.id === placeholderId);
      if (idx >= 0) assetLibrary.splice(idx, 1);
      alert("上传失败：" + file.name + " — " + (translateError(e.message) || e.message));
    } finally {
...
async function whitelistAsset(id) {
  ...
  try {
    ...
    if (data.asset) {
      ...
      // Also update mediaItems if matched
...
function insertSavedAsset(id) {
  ...
  const typeLabel = existing.type === "image" ? "图片" : existing.type === "video" ? "视频" : "音频";
  let count = 0;
  for (const m of mediaItems) {
...
// Returns true if user already has a key; otherwise shows modal and returns false.
